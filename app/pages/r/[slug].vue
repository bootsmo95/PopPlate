	...
	<div class="r-page">
		...
		<main v-if="pending" class="wrap animate-pulse">
			<header class="r-header">
				...
				<div
					class="h-[clamp(72px,11vw,180px)] w-[70%] rounded mb-9"
				...
				<div v-for="i in 3" :key="i" class="r-dish" style="cursor: default">
					<div class="r-dish-text">
						...
						<div class="flex justify-between gap-6 mb-5 pb-5 border-b border-dashed border-ink/10">
							<div
								class="h-8 rounded"
								:style="{ width: ['65%', '50%', '75%'][i - 1], background: 'rgba(26, 20, 16, 0.07)' }"
							/>
		...
		<div v-else-if="error || !menu" class="flex min-h-screen items-center justify-center px-6">
			<div class="max-w-sm text-center">
				...
				<p class="mt-4 text-[15px] text-ink-mute leading-relaxed">
					Denne menu er muligvis ikke publiceret endnu.
				</p>
			</div>
		</div>
					...
					<div class="r-dish-3d">
						...
						<div v-else class="r-dish-placeholder">
							<svg
								...
								stroke-linecap="round"
								stroke-linejoin="round"
								style="color: rgba(212, 168, 128, 0.4)"
							>
								<path
									d="M21 7.5l-9-5.25L3 7.5m18 0l-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9"
								/>
	...
	meta: [
		{
			name: "description",
			content: computed(() =>
				menu.value ? `Se ${menu.value.restaurant.name}s 3D-menu på popplate.` : "Se en 3D-menu på popplate.",
			),
		},
	],
});
...
.r-dish:hover {
	background: rgba(184, 122, 78, 0.04);
}
.r-dish.selected {
	background: rgba(184, 122, 78, 0.06);
}
@media (max-width: 800px) {
	.r-dish {
...
.r-dish-head {
	...
	margin-bottom: 18px;
	padding-bottom: 18px;
	border-bottom: 1px dashed rgba(26, 20, 16, 0.1);
}
.r-dish-head h3 {
	font-family: theme("fontFamily.display");
	font-weight: 400;
