...
<template>
  <div data-screen-label="Settings">
        ...
        <div>
          <div class="p-card">
            <div class="flex items-center gap-5 mb-8">
              <span class="grid place-items-center w-16 h-16 rounded-full bg-clay text-white font-display italic text-2xl shrink-0">
                {{ (profileDisplayName || user?.email)?.charAt(0)?.toUpperCase() ?? '?' }}
              </span>
              <div>
                <div class="font-display text-[22px] font-normal tracking-[-0.015em]">{{ profileDisplayName || user?.email }}</div>
                <div class="mono-label !text-clay-deep mt-1">{{ user?.accountTier?.toUpperCase() ?? 'FREE' }}</div>
              </div>
            </div>
            ...
            <form @submit.prevent="saveProfile">
              ...
              <div class="mb-6">
                ...
                <div class="flex items-center gap-3">
                  <span class="status-badge status-badge--published">{{ user?.accountTier ?? 'free' }}</span>
                  <button type="button" class="text-clay-deep font-mono text-[11px] uppercase font-medium tracking-[0.15em]" @click="tab = 'tier'">
                    Aendr plan &rarr;
                  </button>
                </div>
              </div>
    ...
    <div v-if="tab === 'team' || tab === 'api'" class="p-card relative overflow-hidden p-16 text-center" style="background: #2b1f15; border: none;">
      <div class="absolute inset-0 pointer-events-none" style="background: radial-gradient(circle at 50% 40%, rgba(184, 122, 78, 0.3), transparent 60%);" />
      <div class="relative">
        ...
        <p class="text-[15px] max-w-[420px] mx-auto" style="color: rgba(243, 237, 226, 0.6);">
          {{ tab === 'team' ? 'Inviter teammedlemmer og tildel roller. Under udvikling.' : 'Integrer popplate med jeres egne systemer. Under udvikling.' }}
        </p>
      </div>
    </div>
  </div>
