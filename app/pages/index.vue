...
function useCountUp(target: number, decimals = 0) {
	...
	const display = computed(() => (decimals > 0 ? value.value.toFixed(decimals) : Math.round(value.value).toString()));
...
<template>
	<main data-screen-label="Landing">
		...
		<section class="relative pt-20 pb-16 overflow-hidden max-[720px]:pt-16 max-[720px]:pb-10">
			<div
				class="absolute inset-0 pointer-events-none z-0 opacity-50"
				style="
					background:
						radial-gradient(ellipse 1200px 700px at 50% 20%, rgba(212, 168, 128, 0.35), transparent 65%),
						radial-gradient(ellipse 800px 600px at 20% 80%, rgba(139, 78, 44, 0.18), transparent 60%);
				"
			/>
			...
			<div class="wrap">
				<div class="section-head" data-reveal>
					...
					<p class="section-aside">
						Vi skjuler den tunge maskine — 3D-rekonstruktion, mesh-cleanup, materialer, AR-eksport — bag en
						enkelt knap. I uploader fotos. Vi leverer en menu der lever.
					</p>
				</div>
				...
				<div class="flow">
					...
					<div class="flow-card" data-reveal>
						...
						<div class="flow-visual">
							<div class="stack">
								<div class="stack-img">
									<div class="photo" style="background: linear-gradient(135deg, #d4a880, #8b6e4a)" />
								</div>
								<div class="stack-img">
									<div class="photo" style="background: linear-gradient(135deg, #c89968, #8b4e2c)" />
								</div>
								<div class="stack-img">
									<div class="photo" style="background: linear-gradient(135deg, #b87a4e, #6e4a45)" />
								</div>
							</div>
						</div>
								...
								<svg viewBox="0 0 200 200">
									...
									<g fill="none" stroke="url(#meshGrad)" stroke-width="0.6" opacity="0.85">
										<ellipse
											...
											cx="100"
											:cy="100 + (i - 7.5) * 8"
											:rx="Math.sqrt(Math.max(0, 75 * 75 - ((i - 7.5) * 8) ** 2))"
											:ry="Math.sqrt(Math.max(0, 75 * 75 - ((i - 7.5) * 8) ** 2)) * 0.25"
										/>
										<ellipse
											v-for="i in 12"
											:key="'v' + i"
											...
											rx="75"
											ry="75"
											:transform="`rotate(${(i / 12) * 180} 100 100) scale(${Math.cos((i / 12) * Math.PI)} 1)`"
										/>
									</g>
					...
					<div class="flow-card" data-reveal data-reveal-delay="2">
						...
						<p class="flow-p">
							Én QR-kode per ret — eller én for hele menuen. Virker på alle nyere telefoner uden app.
						</p>
					</div>
				</div>
			</div>
		...
		<section ref="statsRef" class="section">
			<div class="wrap">
				<div class="stats" data-reveal>
					...
					<div class="stat">
						<div class="stat-num">
							<span class="ital">{{ stat2.display.value }}</span
					...
					<div class="stat">
						<div class="stat-num">
							<span class="ital">{{ stat4.display.value }}</span
							><span class="sub-unit">k</span>
						</div>
		...
		<section class="dishes-section" id="menu">
			<div class="wrap">
				<div class="section-head" data-reveal>
					...
					<p class="section-aside">
						Et glimt af hvad jeres gæster ser. På den rigtige menu kan de holde telefonen op til bordet — og
						retten dukker op i fuld størrelse.
					</p>
				</div>
				...
				<div class="dish-feature" data-reveal>
					...
					<div>
						...
						<h3 class="dish-feature-title">
							Brændt porre, brunet smør & <span class="italic text-clay-deep">rugkrydder</span>
						</h3>
						<p class="dish-feature-desc">
							Et af de signature-billeder vores AI brugte til at bygge en interaktiv 3D-model. Gæsten kan
							dreje retten, zoome ind på sprødheden, og se den i fuld størrelse på bordet via AR.
						</p>
						<div class="meta">
				...
				<div class="dish-row" data-reveal>
					<div v-for="(d, i) in DISHES" :key="i" class="dish-card">
						...
						<h4 class="dish-card-name">
							{{ d.name.split(d.italic)[0] }}<span class="italic text-clay-deep">{{ d.italic }}</span
							>{{ d.name.split(d.italic)[1] || "" }}
						</h4>
		...
		<section class="moodboard-section">
			<div class="wrap">
				<div class="section-head" data-reveal>
					...
					<p class="section-aside">
						popplate er bygget til steder hvor præsentationen betyder noget. Hvor lyssætningen er overvejet,
						og hvor gæsten husker retten lige så meget som smagen.
					</p>
				</div>
				...
				<div class="moodboard-grid" data-reveal>
					...
					<div class="mood-img m2">
						<img :src="IMG.mood1" alt="Atmosphere" loading="lazy" /><span class="mood-caption"
							>02 — Atmosphere</span
						>
					</div>
					<div class="mood-img m3">
						<img :src="IMG.mood2" alt="Detail" loading="lazy" /><span class="mood-caption"
							>03 — Detail</span
						>
					</div>
					<div class="mood-img m4">
						<img :src="IMG.mood3" alt="Craft" loading="lazy" /><span class="mood-caption">04 — Craft</span>
					</div>
					<div class="mood-img m5">
						<img :src="IMG.mood4" alt="Setting" loading="lazy" /><span class="mood-caption"
							>05 — Setting</span
						>
					</div>
					<div class="mood-img m6">
						<img :src="IMG.mood5" alt="Pairing" loading="lazy" /><span class="mood-caption"
							>06 — Pairing</span
						>
					</div>
				</div>
		...
		<section class="section section-alt" id="priser">
			<div class="wrap">
				<div class="section-head" data-reveal>
					...
					<p class="section-aside">
						Vælg en plan. Skift eller annullér når som helst. Ingen opsætningsgebyrer, ingen overraskelser
						på regningen.
					</p>
				</div>
				...
				<div class="pricing-grid" data-reveal>
					<div v-for="t in TIERS" :key="t.name" class="tier" :class="{ featured: t.featured }">
						...
						<div class="tier-price">
							{{ t.price }}<span class="sub"> {{ t.sub }}</span>
						</div>
			...
			<div class="wrap">
				<div class="cta-card" data-reveal>
					...
					<div class="cta-content">
						...
						<p class="cta-sub">
							Opret konto, upload tre billeder af én ret, og se den i AR. Ingen kreditkort. Ingen
							kontrakt. Bare en QR-kode I kan vise gæsterne i aften.
						</p>
						<div class="cta-actions">
							...
							<NuxtLink to="/kontakt" class="btn-ghost cta-btn-ghost">Book demo</NuxtLink>
						</div>
					</div>
...
.hero-centerpiece {
	...
	aspect-ratio: 1 / 1.15;
	box-shadow:
		0 60px 120px rgba(26, 20, 16, 0.32),
		0 20px 40px rgba(26, 20, 16, 0.18);
	background: radial-gradient(circle at 50% 35%, #d4a880 0%, #b87a4e 30%, #8b4e2c 75%, #1a1410 100%);
}
@media (max-width: 720px) {
	.hero-centerpiece {
		width: clamp(200px, 78vw, 380px);
	}
}
...
.flow-card:hover .stack-img:nth-child(1) {
	transform: rotate(-14deg) translate(-26px, -12px);
}
.flow-card:hover .stack-img:nth-child(2) {
	transform: rotate(-4deg) translate(-10px, 4px);
}
.flow-card:hover .stack-img:nth-child(3) {
	transform: rotate(10deg) translate(20px, -6px);
}
...
.flow-card:hover .ar-phone {
	transform: perspective(900px) rotateY(-14deg) rotateX(4deg) translateY(-4px);
}
.ar-phone::before {
	content: "";
	position: absolute;
...
.ar-dish-3d {
	...
	background:
		...
		radial-gradient(
			circle at 60% 60%,
			theme("colors.clay.deep"),
			theme("colors.clay.DEFAULT") 60%,
			theme("colors.clay.soft")
		);
	box-shadow:
...
@keyframes float {
	...
	50% {
		transform: translateY(-10px) rotateX(58deg);
	}
}
.ar-corner {
	position: absolute;
...
.featured .tier-cta:hover {
	background: theme("colors.clay.deep");
	border-color: theme("colors.clay.deep");
}
...
.cta-btn-primary:hover {
	background: theme("colors.ink.inv") !important;
}
.cta-btn-ghost {
