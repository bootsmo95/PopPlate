		...
		<template v-if="dish">
			...
			<div class="px-8 pt-10 pb-32 max-[600px]:px-5 max-[600px]:pt-8 max-[600px]:pb-36">
				...
				>
					<template v-if="dish.italic">
						{{ splitItalic(dish)[0] }}<span class="italic text-clay-deep">{{ splitItalic(dish)[1] }}</span
						>{{ splitItalic(dish)[2] }}
					</template>
				...
				<div class="mb-7" v-if="dish.ingredients?.length">
					...
					<div class="flex flex-wrap gap-2">
						<span
							v-for="i in dish.ingredients"
							:key="i"
							class="font-body text-[13px] px-3.5 py-1.5 bg-card rounded-full text-ink-soft"
							>{{ i }}</span
						>
					</div>
				</div>
			...
			<div
				...
				style="
					...
					background: linear-gradient(to bottom, rgba(243, 237, 226, 0), #f3ede2 30%);
				"
			>
				<NuxtLink
					:to="`/d/${dish.id}`"
