...
<template>
	<!-- Mobile top bar -->
	<div
		class="platform-mobile-top max-h-[60px] sticky top-0 z-40 hidden items-center justify-between border-b border-line px-4 py-2.5"
		style="
	...
	>
		<NuxtLink
			to="/"
			class="inline-flex items-center gap-3 font-display italic font-medium text-[18px] tracking-tight text-ink"
		>
			<LogoMark :size="24" />
			<span>popplate</span>
		</NuxtLink>
	...
	<aside
		class="platform-sidebar bg-paper border-r border-line p-7 px-[22px] flex flex-col sticky top-0 h-screen overflow-y-auto z-50"
		:class="open ? 'open' : ''"
	...
	>
		...
		<nav class="flex flex-col gap-0.5 mb-3.5">
			...
			<NuxtLink
				to="/platform/dishes"
				class="nav-item"
				:class="active === 'dishes' && 'active'"
				:aria-current="active === 'dishes' ? 'page' : undefined"
				@click="close"
			>
			...
			<NuxtLink
				...
				class="nav-item"
				:class="active === 'restaurants' && 'active'"
				:aria-current="active === 'restaurants' ? 'page' : undefined"
				@click="close"
			>
				<Icon name="restaurant" /> Restauranter
			</NuxtLink>
			<NuxtLink :to="publicMenuPath" class="nav-item" @click="close"> <Icon name="qr" /> Menu </NuxtLink>
		</nav>
		...
		<nav class="flex flex-col gap-0.5">
			...
			<NuxtLink to="/pricing" class="nav-item" @click="close"> <Icon name="dish" /> Planer </NuxtLink>
		</nav>
		...
		<div class="mt-auto pt-4 border-t border-line">
			<div class="flex items-center gap-2">
				...
				>
					<span
						class="grid place-items-center w-9 h-9 rounded-full bg-clay text-white font-display italic text-base shrink-0"
					>
						{{ userInitials || userName?.charAt(0) || "?" }}
					</span>
					<span class="flex-1 text-left min-w-0">
						<span class="block text-[13px] font-medium text-ink truncate">{{
							userName || userEmail || "User"
						}}</span>
						<span class="block text-[11px] text-ink-faint font-mono uppercase tracking-wider">{{
							accountTier
						}}</span>
					</span>
				</NuxtLink>
...
@media (max-width: 900px) {
	...
	.platform-scrim:not([style*="pointer-events: none"]) {
		display: block;
	}
}
